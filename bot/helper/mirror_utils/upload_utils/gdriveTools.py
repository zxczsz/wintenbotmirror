...
class GoogleDriveHelper:
    ...
    def drive_list(self, fileName):
        ...
        content_count = 0
        if response["files"]:
            msg += f'<h4>Results : {fileName}</h4><br>'

            for file in response.get('files', []):
                if file.get('mimeType') == "application/vnd.google-apps.folder":  # Detect Whether Current Entity is a Folder or File.
                    furl = f"https://drive.google.com/drive/folders/{file.get('id')}"
                    msg += f"📄 File Name : <code>{file.get('name')}<br>📥 Total Size : -\n🏷 Type : Folder 📁</code><br>🔗 Link :-<br>"
                    if SHORTENER is not None and SHORTENER_API is not None:
                    ...
                    msg += f"📄 File Name : <code>{file.get('name')}</code><br>📥 Total Size : {get_readable_file_size(int(file.get('size')))}<br>🏷 Type : File 📄<br>🔗 Link :-<br>"
                    if SHORTENER is not None and SHORTENER_API is not None:
                        sfurl = requests.get(f'https://{SHORTENER}/api?api={SHORTENER_API}&url={furl}&format=text').text
                        msg += f"<b><a href={sfurl}>⚡ Google Drive ⚡</a></b>"
                    else:
